...
def process_import(self, job_id):
    ...
    try:
        ...
        # we can find out / filter product based on sku field (based on sku field of each item in the Excel sheet)
        existing_products = {
            p.sku: p for p in Product.objects.all()
        }
        ...
        for row_number, row in enumerate(ws.iter_rows(min_row=2, values_only=True), start=2):
            ...
            if len(batch) == BATCH_SIZE:
                ...
                job.save(update_fields=["processed_rows", "processed_batches"])
